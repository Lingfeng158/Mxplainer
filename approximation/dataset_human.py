...
def workload(path_to_data, file_name, is_training):
    ...
    with open(path_to_file, "rb") as f:
        ...
        if is_training:
            if len(meta_list) > 2:
                cache["meta"] = meta_list[1:-1]
                cache["raw_meta_feature"] = raw_meta_feature_list[1:-1]
                cache["tile_wall_feature"] = tile_wall_feature_list[1:-1]
                cache["discard_history3"] = discard_history[1:-1]
                cache["pack_info3"] = pack_info[1:-1]
                cache["data_holder"] = data_list[1:-1]
                cache["label_action"] = label_action_list[1:-1]
                cache["label_tile"] = label_tile_list[1:-1]
        else:
            cache["meta"] = meta_list
            cache["raw_meta_feature"] = raw_meta_feature_list
            cache["tile_wall_feature"] = tile_wall_feature_list
            cache["discard_history3"] = discard_history
            cache["pack_info3"] = pack_info
            cache["data_holder"] = data_list
            cache["label_action"] = label_action_list
            cache["label_tile"] = label_tile_list
...
class MahjongSLDataset(Dataset):
    def __init__(self, path_to_data, begin=0, end=1, is_training=True):
        self.match_samples = os.listdir(path_to_data)[9000:34000]
