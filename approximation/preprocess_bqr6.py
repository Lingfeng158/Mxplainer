...
def find_missing_tile(hand_list, selected_tiles):
    missing_tile = defaultdict(default_zero)
    for t in hand_list:
...
def calculate_target_probability_differentiable_prep(
    ...
    missing_tile_list = find_missing_tile(hand_list, list_element[1])

    return tile_base, fan_base, missing_tile_list, (count_trio, count_straight)
...
def summarize_data_prep(
    ...
    for list_element in segmented_list:
        ...
        missing_tile_prep = convert_data_representation(
            missing_tile_list, tile_list_raw, 0
        )
...
if __name__ == "__main__":
    cpuCount = os.cpu_count() - 24
    path = "download_BQR_data/data"
    dst = "sl_prep_simple_BQR_rev6"
