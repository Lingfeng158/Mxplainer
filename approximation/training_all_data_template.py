from dataset_v4 import MahjongSLDataset
from model_v8 import MahJongNetBatchedRevised
import torch
...
def workload(
    ...
    for i, d in enumerate(
        ...
        (
            ...
            meta.to(device),
            meta_feature_new.to(device).float(),
            tile_wall_feature.to(device).float(),
            # data_description.to(device).float(),
            fan_summary.to(device).float(),
            label_action.to(device).float(),
            label_tile.to(device).float(),
        )
        (k, q, m, n, o) = data1
        (k, q, m, n, o) = (
            k.to(device).float(),
        ...
        x = (meta_feature_new, tile_wall_feature, data1, fan_summary)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        config = {
            ...
            "num_dataloader": 4,
            "data_folder_name": "sl_prep_slim6_2k",
            "note": "model_v8_2k",
        }
        print(config)
