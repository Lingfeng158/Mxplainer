...
class TileProbModuleNewS(nn.Module):
    """
    Computation for opponent_desire_prob, held_prob, discard_prob
    """

    def __init__(self):
        super(TileProbModuleNewS, self).__init__()
        self.cnn_channel = 4
        self.linear_output = 4
        self.tile_nn = nn.Sequential(
            nn.Conv1d(9, self.cnn_channel, 3, stride=1),
            nn.MaxPool1d(3),
        )
    ...
    def forward(self, meta_feature, tile_wall_feature):
        # input: meta (data[:4])
        tile_wall_feature_flatten = tile_wall_feature.view(-1, 9, 5)
...
class TileProbModuleNew(nn.Module):
    ...
    def forward(self, meta_feature, tile_wall_feature):
        # input: meta (data[:4])
        tile_wall_feature_select = tile_wall_feature[:, :, :, 2]
        tile_wall_feature_flatten = tile_wall_feature_select.view(-1, 34 * 9)
        feature_concat = torch.cat([tile_wall_feature_flatten, meta_feature], dim=1)
        final = self.seq(feature_concat)
        return final.view(-1, 34)
...
class MahJongNetBatchedRevised(nn.Module):
    ...
    def __init__(self, device="cpu"):
        ...
        self.device = device
        self.fan_coeff_ops = nn.Parameter(
            torch.tensor(
                [1.0 + torch.randn(1) * 0.015 for _ in range(80)], requires_grad=True
            )
        )
        ...
        self.trio_coeff = nn.Parameter(torch.tensor(1.0, requires_grad=True))
        self.straight_coeff = nn.Parameter(torch.tensor(1.0, requires_grad=True))
        self.QQR_pack_penalty = nn.Parameter(torch.tensor(0.1, requires_grad=True))
        self.chi_peng_coeff = nn.Parameter(
            torch.tensor(
                [1.0 + torch.randn(1) * 0.015 for _ in range(5)], requires_grad=True
            )
        )

        self.chi_peng_prob_coeff = nn.Parameter(
            torch.tensor(
                [
                    0.1 + torch.randn(1) * 0.015,
                    1.0 + torch.randn(1) * 0.015,
                    3.0 + torch.randn(1) * 0.015,
                ],
                requires_grad=True,
            )
        )
        ...
        self.prob_module_throw_cp = TileProbModuleNewS().to(device)
        self.prob_module_operations = TileProbModuleNew().to(device)

    def show_fan_params(self):
        fan_dict = reversed_tile_conversion(
            self.fan_coeff.detach().numpy().tolist(), fan_list_raw
        )
        sorted_fan = sorted(fan_dict.items(), key=lambda x: -x[1])
        return fan_dict, sorted_fan

    def access_fan_params(self):
        return self.fan_coeff_throw

    def show_tile_params(self):
        tile_dict = reversed_tile_conversion(
            self.tile_coeff.detach().numpy().tolist(), tile_list_raw
        )
        sorted_tile = sorted(tile_dict.items(), key=lambda x: -x[1])
        return tile_dict, sorted_tile

    def access_tile_params(self):
        return self.tile_coeff

    def show_standalone_params(self, show=True):
        if show:
            print(
                "Trio_coeff: {:.3f}, Straight_coeff: {:.3f}, 全求人惩罚项: {:.3f}".format(
                    self.trio_coeff.item(),
                    self.straight_coeff.item(),
                    self.QQR_pack_penalty.item(),
                )
            )
        return {
            "Trio_coeff": self.trio_coeff.item(),
            "Straight_coeff": self.straight_coeff.item(),
            "QQR_penalty": self.QQR_pack_penalty.item(),
        }
    ...
    def forward(self, x):
        ...
        prob_throw_cp = self.prob_module_throw_cp(meta_feature, tile_wall_feature)
        prob_ops = self.prob_module_operations(meta_feature, tile_wall_feature)
        ...
        missing_tile_prep_throw_data = missing_tile_prep_data[:, 0, :, :]
        chi_peng_count_remain_throw_data = chi_peng_count_remain_data[:, 0, :]
        tile_prep_throw_data = tile_prep_data[:, 0, :, :, :]
        tile_self_draw_baseline_throw = tile_prep_throw_data[:, :, :, 0]
        tile_need_count_throw = tile_prep_throw_data[:, :, :, 1]
        tile_cp_coeff_throw = tile_prep_throw_data[:, :, :, 2]
        tile_cp_base_throw = tile_prep_throw_data[:, :, :, 3]
        tile_hidden_ct_throw = tile_prep_throw_data[:, :, :, 4]
        tile_cp_switch_throw = tile_prep_throw_data[:, :, :, 5]
        tile_null_filler_throw = tile_prep_throw_data[:, :, :, 6]
        ...
        # Throw operation
        # fan preference
        fan_prep_throw = fan_prep[:, 0, :, :]
        fan_prep_throw = fan_prep_throw.unsqueeze(-1)
        tile_pref_from_fan = torch.sum(fan_prep_throw * self.fan_coeff_throw, dim=-2)

        one_hot_tile_cp_coeff = F.one_hot(tile_cp_coeff_throw.long(), num_classes=3)
        tile_cp_multiplier = torch.sum(
            one_hot_tile_cp_coeff * self.chi_peng_prob_coeff, dim=-1
        )

        # tile probability (throw)
        term1_throw = (
            tile_self_draw_baseline_throw
            * prob_throw.unsqueeze(1) ** tile_need_count_throw
        )
        term2_throw = tile_cp_base_throw * tile_cp_multiplier / tile_hidden_ct_throw  #
        term2_throw_zeroed = torch.nan_to_num(term2_throw)
        term3_throw = tile_null_filler_throw
        terms_summed_throw = (
            term1_throw
            + term3_throw
            + term2_throw_zeroed * tile_cp_switch_throw * prob_throw_cp.unsqueeze(1)
        )
        multiplied_sum_throw = torch.prod(terms_summed_throw, -1) * 100
        final_prob_throw = (
            multiplied_sum_throw
            * self.QQR_pack_penalty**chi_peng_count_remain_throw_data
        )

        # distinguish by missing tile (Throw)

        missing_tile_prob_throw = (
            missing_tile_prep_throw_data
            * tile_pref_from_fan
            * final_prob_throw.unsqueeze(-1)
        )
        ...
        tile_bound_winrate_throw = missing_tile_prob_throw.sum(dim=1)

        # final tile-bounded prob
        weighted_tile_prob = (
            tile_bound_winrate_throw * self.tile_coeff
        )  # - opponent_desire_prob.unsqueeze(1)

        # tile probability (operations)
        # fan preference
        fan_pref_ops = fan_prep * self.fan_coeff_ops + fan_prep_prime
        multiplied_fan_pref_ops = torch.prod(fan_pref_ops, dim=-1).view(
            -1, 320
        )  # n * 5 * 64
        term1_ops = tile_self_draw_baseline * prob_ops.unsqueeze(1) ** tile_need_count
        # term2 = tile_prep3 * held_prob_unsqueezed * discard_prob_unsqueezed
        term3_ops = tile_null_filler
        terms_summed_ops = term1_ops + term3_ops
        multiplied_sum_ops = (
            torch.prod(terms_summed_ops, -1) * multiplied_fan_pref_ops * 100
        )
        final_prob_ops = (
            multiplied_sum_ops * self.QQR_pack_penalty**chi_peng_count_remain
        )

        # distinguish by missing tile (Operations)

        missing_tile_prob_ops = missing_tile_prep * final_prob_ops.unsqueeze(-1)

        # combine 64 sub entries
        # reshape
        # final_prob: win rate for each 64 entries
        final_prob_ops = final_prob_ops.view(-1, 5, 64)
        # win-rate transfered to tile, judge tile by tile-bounded win-rate
        missing_tile_prob_ops = missing_tile_prob_ops.view(-1, 5, 64, 34)
