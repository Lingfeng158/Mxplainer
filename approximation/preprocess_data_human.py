...
def calculate_target_probability_differentiable_prep(
    ...
    return probability, missing_tile
    """
...
def summarize_data_prep(
    ...
    for list_element in segmented_list:
        ...
        redundant_tile_prep = convert_data_representation(
        ...
        if "全求人" in list_element[5]:
            ...
            data_prep_collector.append(
                (tile_prep, fan_prep, redundant_tile_prep, coeff_count, 0)
            )
...
def prepare_data(path_to_data, data_name, data_dst, meta_info_dict, target_id_list):
    ...
    for target_id in target_id_list:
        ...
        for line_id in range(5, len(botzone_log) - 2):
            ...
            discard_history_dict_list_slow = copy.deepcopy(
                discard_history_dict_list_adv
            )
            ...
            if (
                keywords[2] == "Play"
                or keywords[2] == "BuGang"
                or keywords[2] == "AnGang"
            ):
                for player_id_offset in range(4):
                    player_id = (int(keywords[1]) + player_id_offset) % 4

                    # if player_id is not target_id_player_id, then skip
                    if player_id != target_id_player_id:
                        continue
                    ...
                    prev_hand = defaultdict(default_zero, prev_hand)
                    ...
                    if player_id_offset == 0:
                        ...
                        for offset in range(4):
                            ...
                            play_history = discard_history_dict_list_slow[
                                other_player_id
                            ]
                        ...
                        if case_prep_contains_useful_values == False:
                            ...
                            truth_label_list = [
                                one_hot_truth_action,
                                one_hot_truth_tile,
                            ]
                            truth_label_gatherer.append(truth_label_list)
                            verification_label_gatherer.append(
                                [truth_action, truth_tile]
                            )
                            data_info_gatherer.append([data_name, line_id, player_id])
                            # process other players' discard history and shown pack
                            # should contain other players' histories
...
if __name__ == "__main__":
    ...
    dst = "sl_prep_human_rev6_"
    id_list = ["KRWilma", "#sbot1"]
    for id in id_list:
