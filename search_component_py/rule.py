...
def one_type_dist_to_straight(type, hand_list, tile_list):
    ...
    for rank in range(2, 9):
        ...
        mid_dist = (
            ...
            else 1 if scratch_place[midkey]["remain"] > 0 else 9
        )
        low_dist = (
            0
            if scratch_place[lowkey]["hand"] > 0
            else 1 if scratch_place[lowkey]["remain"] > 0 else 9
        )
        high_dist = (
            0
            if scratch_place[highkey]["hand"] > 0
            else 1 if scratch_place[highkey]["remain"] > 0 else 9
        )
...
def update_hand_and_tile_info(hand_list, tile_list, tile_appearance_list):
    """
    return new hand_list and tile_list from given hand_list, tile_list, and tile_appearance_list
    """
    hand_list_ret = hand_list.copy()
    tile_list_ret = tile_list.copy()
    pack = tile_appearance_list.get("shown", [])
    for entry in pack:
        for t in entry:
            if t != "AnGang":
                if entry[t] > hand_list.get(t, 0):
                    hand_list_ret[t] = 0
                    tile_list_ret[t] -= entry[t] - hand_list.get(t, 0)
                else:
                    hand_list_ret[t] = hand_list[t] - entry[t]
    for t in tile_appearance_list:
        if t != "shown":
            if tile_appearance_list[t] > hand_list.get(t, 0):
                hand_list_ret[t] = 0
                tile_list_ret[t] -= tile_appearance_list[t] - hand_list.get(t, 0)
            else:
                hand_list_ret[t] = hand_list[t] - tile_appearance_list[t]
    return hand_list_ret, tile_list_ret
...
def calc_fan_with_PyMahJongGB_quick(
    ...
    for i in range(len(canonical_incomplete_list)):
        ...
        if is_WFH:
            contains_WFH = True
            WFH_last_tile_list.append(last_tile)

        if fan_sum < min_fan:
            min_fan = fan_sum
        if fan_sum > max_fan:
            max_fan = fan_sum
        if fan_sum >= target_fan_val:
            last_tile_selection.append(last_tile)
        ...
        for fan in ret1:
            if fan[1] in ["不求人", "双暗刻", "双暗杠", "暗杠", "门前清"]:
                rectified_fan -= fan[0]
        if fan_sum >= target_fan_val and rectified_fan < target_fan_val:
            free_chi_peng = False
        ...
        for fan in ret1:
            if fan[1] in ["边张", "嵌张", "单钓将"]:
                rectified_fan -= fan[0]
        if rectified_fan < target_fan_val:
            free_hu = False
    if contains_WFH:
        return (True, True, False, list(set(WFH_last_tile_list)))
    if min_fan >= target_fan_val and free_chi_peng == True:
        return (True, True, True, [])
    if max_fan < target_fan_val:
