...
# Note:
# Based on v1, this bot:
# B1. fixed crash when search-result list is empty (Done)
# B2. memorized other's play history and pack info
# B3. is able to load parameter from supervised_learning_v2 model

# Bot v3:
# Based on v2, this bot:
# C1. deal with 全求人, with additional search on 2-fan targets
# C2. modified search-result logic accomodating 全求人
# C3. is able to load parameter from supervised_learning_v3 model

# In DEVELOPMENT, pending feature C3
# Currently Working On: C3
...
class TileProbModule(nn.Module):
    ...
    def forward(self, meta_feature, tile_wall_feature):
        # input: meta (data[:4])
        tile_wall_feature_flatten = tile_wall_feature.view(-1, 7, 5)
...
def construct_network_input(
    remaining_tile_ct, opponent_held_ct, tile_wall_info, pack_info, discard_info
):
    """
...
def calculate_target_probability(
    ...
    for k in tile_dict:
        ...
        list_probability *= QQR_penalty**effective_chi_peng_count_remain
...
if __name__ == "__main__":
    ...
    # load params
    # path_to_params = '/data/extracted_params'
    path_to_params = "./supervised_learning_v3/extracted_params"
    held_prob_module = TileProbModule()
    discard_prob_module = TileProbModule()
    opponent_desire_prob_module = TileProbModule()
    held_prob_module.load_state_dict(
        torch.load("{}/hp.pkl".format(path_to_params), map_location=torch.device("cpu"))
    )
    discard_prob_module.load_state_dict(
        torch.load("{}/dp.pkl".format(path_to_params), map_location=torch.device("cpu"))
    )
    opponent_desire_prob_module.load_state_dict(
        torch.load(
            "{}/odp.pkl".format(path_to_params), map_location=torch.device("cpu")
        )
    )
    with open("{}/global.json".format(path_to_params), "r") as f:
        tmp = json.load(f)
        fan_param_dict = tmp["fan_param"]
        tile_param_dict = tmp["tile_param"]
        stdal_param_dict = tmp["standalone"]
    # print(fan_param_dict)
    trio_coeff = stdal_param_dict["Trio_coeff"]
    straight_coeff = stdal_param_dict["Straight_coeff"]
    QQR_penalty = stdal_param_dict["QQR_penalty"]
    ...
    while True:
        ...
        if t[0] == "0":
            ...
            if rule.check_hu(
                ...
                hp = (
                    held_prob_module(meta_feature, tile_wall_feature)
                    .data.numpy()
                    .tolist()[0]
                )
                dp = (
                    discard_prob_module(meta_feature, tile_wall_feature)
                    .data.numpy()
                    .tolist()[0]
                )
                odp = (
                    opponent_desire_prob_module(meta_feature, tile_wall_feature)
                    .data.numpy()
                    .tolist()[0]
                )
                hp_dict = reversed_tile_conversion(hp, tile_list_raw)
                dp_dict = reversed_tile_conversion(dp, tile_list_raw)
                odp_dict = reversed_tile_conversion(odp, tile_list_raw)
                (action, eject_tile, max_prob) = ejection(
                    ...
                    prevailing_wind,
                    held_prob=hp_dict,
                    discard_prob=dp_dict,
                    fan_preference=fan_param_dict,
                    tile_preference=tile_param_dict,
                    opponent_desire_prob=odp_dict,
                    opponent_hidden_tiles=opponents_hidden_tile_count,
                    straight_coeff=straight_coeff,
                    trio_coeff=trio_coeff,
                    QQR_penalty=QQR_penalty,
                )
