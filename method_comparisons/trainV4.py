...
import datetime
import os
from MahJong_CNN_model6_largeV4 import MahJongCNNNet6_LargeV4
    ...
    for i, d in enumerate(
        ...
        if is_training:
            ...
            with torch.no_grad():
                ...
                top3_pred = torch.topk(logits, k=3, dim=1).indices

                # Check if the true class is in the top 3 predictions
                correct_top3 += (
                    torch.any(top3_pred == d[2].view(-1, 1).to(device), dim=1)
                    .sum()
                    .item()
                )
...
if __name__ == "__main__":
    # configs
    model = MahJongCNNNet6_LargeV4
    data_path = "/mnt/storage/llf/test/sl_data_featurefull3"
    meta_path = "/mnt/storage/llf/test/count_featurefull3.json"
    # data_path = "../data/smth/sl_data_featurefull3"
    # meta_path = "../data/smth/count_featurefull3.json"

    logdir_base = "logs"
    log_note = "test_featurefull3_normalized"
