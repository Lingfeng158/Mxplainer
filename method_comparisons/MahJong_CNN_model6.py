...
class MahJongCNNNet6(nn.Module):

    def __init__(self, device):
        super(MahJongCNNNet6, self).__init__()
        self.device = device
        self.public_p = nn.Sequential(
            nn.Conv2d(76, 128, 3, 1, padding=(1, 0)),  # 128*4,7
            nn.ReLU(True),
        ...
        self.public_dense_p_layers = PSCN(512, 256, 4)
        ...
        #     nn.Linear(32, 1),
        # )
        self._logits_branch = PSCN(512, 256, 4)
        self._logits_head = nn.Linear(256, 235)
    ...
    def forward(self, input_data):
        ...
        obs_dense = obs[:, :12].view(-1, 432)
        obs_dense = torch.concat([obs_dense[:, :8], obs_dense[:, 36:]], dim=1)
        # obs = obs.reshape(-1, 4, 9)
        # obs = channel_expansion(obs).type(torch.float32)
        # obs = obs.reshape(-1, 48, 4, 9)
        # v_out = self.public_v(obs)
...
if __name__ == "__main__":
    ...
    data = torch.rand(2, 76, 4, 9)
    mask = torch.ones(2, 235)
    net = MahJongCNNNet6("cpu")
    output1 = net({"observation": data, "action_mask": mask})
    # torch.save(net.state_dict, "./samplesize")
    print("Model Output Shape: ", output1.shape)
    print("Model size: ", sys.getsizeof(cPickle.dumps(net.state_dict())) / 1e3, "K")
