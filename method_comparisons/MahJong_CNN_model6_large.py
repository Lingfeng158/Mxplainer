...
class MahJongCNNNet6_Large(nn.Module):

    def __init__(self, device="cpu"):
        super(MahJongCNNNet6_Large, self).__init__()
        self.device = device
        self.public_p = nn.Sequential(
            nn.Conv2d(24, 256, 3, 1, padding=(1, 0)),  # 128*4,7
            nn.ReLU(True),
        ...
        self.public_v = nn.Sequential(
            nn.Conv2d(24, 128, 3, 1, padding=(1, 0)),  # 128*4,7
            nn.ReLU(True),
...
if __name__ == "__main__":
    ...
    data = torch.rand(2, 24, 4, 9)
    mask = torch.ones(2, 235)
    net = MahJongCNNNet6_Large("cpu")
